...
def test_user_data_keys():
    """
    ✅ 測試：確認產生的 user data 包含所有必要欄位
    """
    code, user, meta = generate_user_data()
    assert code == ResultCode.SUCCESS, f"產生失敗，錯誤碼: {code}，meta: {meta}"
...
def test_password_confirm_matches():
    """
    ✅ 測試：確認 password 與 passwordConfirm 欄位一致
    """
    code, user, meta = generate_user_data()
    assert code == ResultCode.SUCCESS, f"錯誤碼: {code}, meta: {meta}"
...
def test_randomness_of_email():
    """
    ✅ 測試：驗證 email 每次產生皆不同（兩次不應相同）
    """
    code1, user1, _ = generate_user_data()
    code2, user2, _ = generate_user_data()
