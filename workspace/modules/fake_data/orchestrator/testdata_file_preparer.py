...
def prepare_testdata_files(uuid: str) -> Tuple[int, Optional[dict], Optional[dict]]:
    ...
    # 如果路徑生成失敗（格式錯誤）
    if not user_path:
        code = ResultCode.get(meta_u.get("reason", ""), ResultCode.USER_TESTDATA_FILE_WRITE_FAILED)
        return code, None, meta_u
    if not product_path:
        code = ResultCode.get(meta_p.get("reason", ""), ResultCode.TESTDATA_PRODUCT_FILE_WRITE_FAILED)
        return code, None, meta_p

    # 檢查是否已存在
    if file_exists(user_path) or file_exists(product_path):
        return ResultCode.get("file_already_exists", ResultCode.USER_TESTDATA_ALREADY_EXISTS), None, {
    ...
    if not success_u:
        ...
        code = ResultCode.get(reason, ResultCode.TESTDATA_USER_FILE_WRITE_FAILED)
        return code, None, meta_u
    ...
    if not success_p:
        ...
        code = ResultCode.get(reason, ResultCode.TESTDATA_PRODUCT_FILE_WRITE_FAILED)
        return code, None, meta_p
